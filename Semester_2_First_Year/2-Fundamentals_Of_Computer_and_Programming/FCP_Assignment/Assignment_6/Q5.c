...
void main()
{
    ...
    for (i = 0; i <= stringLen - wordLen; i++)
    {
        ...
        if (found == 1)
        {
            for (j = 0; j < repwordLen; j++)
            {
                //Replacing Character by Character
                str[i + j] = Repword[j];
            }
        }
    }
    printf("\nThe String after Replacing : %s ", str);
}
